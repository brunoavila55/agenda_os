...
- ~~atualizar manualmente o catálogo de tipos~~ → catálogo completo (127 tipos reais, não só rural) importado em `migrations/007_real_service_types.sql`, deliberadamente completo para já preparar a futura operação de fibra; usuário escolheu `29`, `30` e `249` como os tipos rurais relevantes — falta só atribuí-los à operação `rural` via `PUT /api/v1/operations/{id}/service-types` (a migração não faz essa atribuição);
...
1. ~~código dos tipos rurais escolhidos~~ → catálogo completo importado; usuário escolheu `29`, `30` e `249` (`AGENTS.md` §6.4); falta atribuí-los à operação via API;

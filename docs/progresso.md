...
- ~~atualizar manualmente o catálogo de tipos~~ → catálogo completo (127 tipos reais, não só rural) importado em `migrations/007_real_service_types.sql`, deliberadamente completo para já preparar a futura operação de fibra; usuário escolheu `29`, `30` e `249` como os tipos rurais relevantes, e eles já estão atribuídos à operação `rural` (testado com `APP_MODE=real` temporário só na API, sem worker, revertido para `simulation` depois);
...
1. ~~código dos tipos rurais escolhidos~~ → catálogo completo importado; usuário escolheu `29`, `30` e `249`, já atribuídos à operação `rural` (`AGENTS.md` §6.4);
...
- `migrations/007_real_service_types.sql` criada e testada contra um PostgreSQL descartável via `docker compose run --rm migrate`: as 127 linhas do catálogo real foram inseridas com `source='real'` sem erro, junto das migrações `001`–`006` já existentes; usuário confirmou que o repositório é público e que esses dados (nomes de setores internos, localidades) podem ficar expostos — só o `.env` (credenciais) continua fora do controle de versão;
- consultada de novo, com perguntas mais específicas, a documentação oficial do MK (as únicas duas páginas que existem, confirmado pelo usuário): confirmado que não há mais nada a extrair — `APIs gerais` só cobre configuração do Webservice Manager, e `APIs especiais` diz literalmente que os webservices alem do básico exigem contato comercial com a MK. Não repetir essa busca esperando resultado diferente (ver `docs/integracao-mk.md`);
- `APP_MODE=real` ligado temporariamente **só na API** (sem subir o `worker`, que continua rejeitando modo real) para atribuir os tipos `29`/`30`/`249` (source `real`) à operação `rural` via `PUT /api/v1/operations/{id}/service-types`; confirmado por `GET /api/v1/operations` que os três IDs ficaram persistidos; `.env` revertido para `simulation` e stack derrubada (volume do Postgres preservado, sem `-v`) ao final.
...
2. **MK**: o único caminho adiante para a sincronização real é obter, com o comercial/suporte da MK, o endpoint e o formato da listagem de O.S. por tipo/situação — confirmado que a documentação pública não tem isso e não vai ter (exige contrato comercial). Enquanto isso não vem, vale conferir se o painel "Webservice Manager" do MK mostra o nome de cada `cd_servico` autorizado no perfil (pode revelar o serviço certo sem depender do comercial). Depois de confirmado, o próximo passo natural é implementar essa chamada em `backend/internal/mk/` seguindo o mesmo padrão (contrato testado antes de codar, erros sanitizados, testes com `httptest`).
